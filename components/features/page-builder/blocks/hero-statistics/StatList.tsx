...
import { EMPTY_STAT, type HeroStat } from "./schema";
...
/**
 * The "Statistici" repeater for Hero – Statistics: add / reorder / delete rows,
 * each with Valoare, Etichetă and an optional Descriere scurtă. Co-located with
...
export function StatList({
  ...
  value: HeroStat[];
  onChange: (next: HeroStat[]) => void;
  error?: string;
}) {
  const update = (index: number, patch: Partial<HeroStat>) =>
        ...
        <ul className="space-y-3">
          {value.map((stat, index) => (
            ...
            >
              ...
              <div className="mt-3">
                <label className={labelClass}>Descriere scurtă (opțional)</label>
                <input
                  className={inputClass}
